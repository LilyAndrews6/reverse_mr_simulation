...
#l <- gc %*% gwashits$beta +prs * sqrt(rsq_prs) +u2*b_u2l  ##could this be the issue of sqrt of rsq_prs as beta not 
#same as r due to variance
...
l_for_r0 <- matrix(l,             
  ...
  prob_l <- simulateGP::gx_to_gp(gx=scale(l), h2x=rsq_prs + b_gc0l^2 + b_u1d^2 + b_u2d^2, prev = d_prev) 
 
  d <- rbinom(nid, 1, prob_l) ##is the issue here where we flip meaning of d
  d <- abs(d-1) 
...
mr_dat <- get_effs(phenosubset$d, phenosubset$`c1[, 1]`, geno_mat)
print("Forward MR")
mr(mr_dat, metho=c("mr_ivw", "mr_wald_ratio")) %>% str()
...
phenosubset <- dat$phen[ind, ]
stopifnot(nrow(genosubset) == nrow(phenosubset))
mr_dat <- get_effs(phenosubset$`c0[, 1]`, phenosubset$`c1[, 1]`, geno_mat) 
print("Forward MR")
mr(mr_dat, metho=c("mr_ivw", "mr_wald_ratio")) %>% str()
```
...
dat <- dgmodel(nid=1000, #making large sample size so enough individuals for case control analysis
...
mr(mr_dat, metho=c("mr_ivw", "mr_wald_ratio")) %>% str()
```
For some reason when we look at reverse MR between r0 and disease this come as a negative effect
