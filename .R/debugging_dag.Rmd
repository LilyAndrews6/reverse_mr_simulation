...
#  }
 out_dat <- data.frame()
  ...
  harmonise <- merge(exp_dat, out_dat, by="SNP")
  harmonise$mr_keep <- TRUE
  c_snp <- harmonise[1301,] #select simulated causal SNP
...
reps <- 5
sims <- mclapply(X=1:reps, function(i){
  for(row in 1:nrow(rev_mr_result)){
dat <- dgmodel(nid=1010000, #making large sample size so enough individuals for case control analysis
  ...
  d_prev=6/10000, #glioma prevalence https://www.ncbi.nlm.nih.gov/books/NBK441874/#:~:text=In%20the%20United%20States%2C%20there,the%20least%20malignant%20brain%20tumors.
  #b_lr0=rev_mr_result[row,]$beta,  #adding rev MR results into data generating model
...
dgmodel_check(dat)
print("made dat")
studies <- dgmodel_analysis(dat, ncase=12496, ncontrol=18190, protein_gwas = 3301, rev_mr_result, row, iteration=i)
print("studies")
...
for (prot in rev_mr_result$prot){
  for (sim in 1:reps){
  file <- read.csv(paste0(prot, sim,"_res_obs.csv"))
