...
dgmodel <- function(nid, #making large sample size so enough individuals for case control analysis
  ...
  vp, 
  nsnp,
  b_dx1)  #assuming variance of genetic liability is 1
{
  u1 <- rnorm(nid) 
  u2 <- rnorm(nid) 
gwashits <- tribble(
...
df <- data.frame(rev= rev_p_score, fwd = fwd_p_score, cc=cc_p_score)
...
param  <- expand.grid(
  ...
  b=0.0879946,#c(0.0879946, -0.0354690,0.0497226), 
  rsq_lr0=0.0052923,#c(0.0052923,0.0008683,0.0016068),
  nestedcasecontrol=2000,#c(487,3000),#c(113, 487,593),
  sims=1
)
...
p1_rev <- rev_mr_prot_p1(dat, n_protein_gwas=param$n_prot[i], ncontrol=18190, ncase=12496)
p2_rev <- rev_mr_prot_p2(dat, n_protein_gwas=param$n_prot[i], ncontrol=18190, ncase=12496)
p3_rev <- rev_mr_prot_p3(dat, n_protein_gwas=param$n_prot[i], ncontrol=18190, ncase=12496)
pval_rev <- rbind(p1_rev, p2_rev, p3_rev)
pval_rev <- subset(pval_rev, pval_rev$pval<0.05)
print(pval_rev)
rev_p_score <- sum(pval_rev$b)

p1_fwd <- fwd_mr_prot_p1(dat, n_protein_gwas=param$n_prot[i], ncontrol=18190, ncase=12496)
p2_fwd <- fwd_mr_prot_p2(dat, n_protein_gwas=param$n_prot[i], ncontrol=18190, ncase=12496)
p3_fwd <- fwd_mr_prot_p3(dat, n_protein_gwas=param$n_prot[i], ncontrol=18190, ncase=12496)
pval_fwd <- rbind(p1_fwd, p2_fwd, p3_fwd)
pval_fwd <- subset(pval_fwd, pval_fwd$pval<0.05)
print(pval_fwd)
fwd_p_score <- sum(pval_fwd$b)

p1_cc <- nested_case_control_p1(dat, nestedcase=param$nestedcasecontrol[i], nestedcontrol=param$nestedcasecontrol[i])
p2_cc <- nested_case_control_p2(dat, nestedcase=param$nestedcasecontrol[i], nestedcontrol=param$nestedcasecontrol[i])
p3_cc <- nested_case_control_p3(dat, nestedcase=param$nestedcasecontrol[i], nestedcontrol=param$nestedcasecontrol[i])
cc <- rbind(p1_cc, p2_cc, p3_cc)
cc<- as.data.frame(cc)
pval_cc <- subset(cc, cc$pval<0.05)
print(pval_cc)
cc_p_score <- sum(unlist(pval_cc$bhat))

pscore_df <- data.frame(rev= rev_p_score, fwd = fwd_p_score, cc=cc_p_score)

return(pscore_df) #list(c(rev, pscore_df))
}) 
