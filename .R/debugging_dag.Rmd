...
Creating protein dataset like INTERVAL cohort to look at r0
```{r}
phen <- dat$phen
gen_gr <- dat$geno_gr
prot <- phen[phen$d==0,][1:protein_gwas, ] #first 3301
...
# cov(x,y)/var(y) beta hat estimate
x <- phenosubset$`r0[, 1]`
g <- geno_mat
#beta.exposure <- function(y, g){
#vg <- var(g[,i])
#bhat <- stats::cov(y, g[,i]) / vg
#return(bhat)
#}
gwas_dat <- data.frame()
for (i in 1:ncol(g)){
...
exp_dat <- dplyr::tibble(
  SNP= 1:ncol(g),#sort this 
  exposure= "X",
  id.exposure= "X",
  beta.exposure=gwas_dat$bhat,
  se.exposure=gwas_dat$se,
  pval.exposure=gwas_dat$pval,
  samplesize.exposure=gwas_dat$n,
  units.exposure="SD",
  rsq.exposure=gwas_dat$fval/(gwas_dat$fval+gwas_dat$n-2) #n-2 for degrees of freedom https://analystprep.com/cfa-level-1-exam/quantitative-methods/coefficient-of-determination-and-f-statistic/
)
```

Create GWAS dataset - must be different population to protein
```{r}
...
mr(harmonise, metho=c("mr_ivw", "mr_wald_ratio")) %>% str()
```
