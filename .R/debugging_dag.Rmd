...
l_for_r0 <- matrix(l,             
  ...
  c1 <- gc * b_gcc1 + u2 * b_u2c1 + c0 *b_c0c1 + d * b_dc1 + rnorm(nid, sd=sqrt(1-(b_gcc1)^2 - b_u2c1^2- b_c0c1^2))
  r1 <- gr * sqrt(rsq_gr0) + r0 * b_r0r1 + u1 * b_u1r1 + d * b_dr1 + rnorm(nid, sd=sqrt(1-rsq_gc0-rsq_lr0^2-b_u1r1^2)) 
   phen <- tibble(
...
dat <- dgmodel(nid=1000000, #making large sample size so enough individuals for case control analysis
  ...
  b_u2c1=0, 
  b_u2l=0, 
  b_dr1=1, #non causal marker increases beta 1 in disease
...
TO DO:
Work out how to sort out r0\*b_r0_r1 would set to 0 but then liability is not involved in r1 and causes r1 to be less than r0 most of the time - only d\*b_dr1 should affect disease
Removed scale from gc and gr - check this is okay with Gib
changed b_gcc0 to beta of causal variant divided by 2, same for b_c0l
Changed MR output for r0 to be r0[,1] need to check why this is the case
No influence of liability in c1 in current DGM
Need to work out how to subset gc/gr genotype matrix for case control to be able to run MR - need to count row numbers and subset this from the matrix