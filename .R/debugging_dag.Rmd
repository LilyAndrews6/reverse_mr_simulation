...
for (x in 1:nrow(gwas_dat)){
  ...
  cc<- fast_assoc(y=phenosubset$d, x=phenosubset$r0) %>% as_tibble() %>% mutate(x="prs", y="d") #case control estimate 
   return(list(pval=mr_res, all=all_mr_res, casecontrol=cc))
  }else{
    mr_res <- tribble(
...
dat <- dgmodel(nid=500000, #making large sample size so enough individuals for case control analysis
  ...
  d_prev=1000/10000, #glioma prevalence https://www.ncbi.nlm.nih.gov/books/NBK441874/#:~:text=In%20the%20United%20States%2C%20there,the%20least%20malignant%20brain%20tumors.
  #b_lr0=rev_mr_result[row,]$beta,  #adding rev MR results into data generating model
...
#print("studies")
rev <- rev_mr_prot(dat, n_protein_gwas=35571, ncontrol=18190, ncase=12496, nsnp=99)
print("rev")
rev$all$mr <- "rev_all"
rev$pval$mr <- "rev_pval"
fwd <- fwd_mr_prot(dat, n_protein_gwas=35571, ncontrol=18190, ncase=12496)
print("fwd")
fwd$mr <- "fwd"
rev$fwd <- fwd
